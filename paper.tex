...
%C.~E.~Petrillo$^{1}$,  C.~Tortora$^{1}$, L.~V.~E.~Koopmans$^{1}$, \and G.~Verdoes~Kleijn$^{1}$
...
Eric Jullo,$^{16}$
\newauthor
Gijs Verdoes Kleijn,$^{22}$ 
Jean-Paul Kneib,$^6$
Leon Koopmans,$^{22}$
\newauthor
Fran\c{c}ois Lanusse,$^{20}$ 
Chun-Liang Li,$^{21}$
Quanbin Ma,$^{21}$ 
\newauthor
Martin Makler,$^{19}$
...
Matthew Lightman,$^{15}$
\newauthor
Carlo Enrico Petrillo,$^{22}$
Christoph Sch\"{a}fer,$^6$ 
Alessandro Sonnenfeld,$^5$ 
\newauthor
Amit Tagore,$^7$
Crescenzo Tortora,$^{22}$
Diego Tuccillo,$^{9,10}$
\newauthor
Manuel B. Valent\'in,$^{19}$
and Santiago Velasco-Forero,$^{10}$
\\
% List of institutions
$^{1}$ Departimento di Fisica \& Astronomia, Universit\`a di Bologna, via Gabetti 93/2, 4012 Bologna, Italy \\
$^{2}$ IRAP, Universit\'e de Toulouse, CNRS, UPS, Toulouse, France.\\
...
$^{6}$ \red{Ecole Polytechnique F\'ed\'erale de Lausanne (EPFL)
...
$^{10}$ MINES Paristech, PSL Research University, Centre for Mathematical Morphology, 35 rue \\ Saint-Honore, Fontainebleau, France \\
...
$^{14}${School of Physics and Astronomy, Nottingham University, University Park, Nottingham, NG7 2RD, UK
}\\
$^{15}${JPMorgan Chase, Chicago, IL 60603 U.S.A.}\\
$^{16}$\red{ LAM - Laboratory of Astrophysics of Marseille P\^ole de l'\'Etoile Website of Ch\^ateau-Gombert 
38,} \\ \red{rue Fr\'ed\'eric Joliot-Curie 13388 Marseille cedex 13 FRANCE} \\
$^{17}$\red{Institut d'Astrophysique de Paris (IAP)- 98 bis boulevard Arago - 75014 Paris } \\
$^{18}${Centro Federal de Educa\c{c}\~ao Tecnol\'ogica Celso Suckow da Fonseca, CEP 23810-000,  Itagua\'i, RJ, Brazil}\\
$^{19}${Centro Brasileiro de Pesquisas F\'isicas, CEP 22290-180, Rio de Janeiro, RJ, Brazil}\\
$^{20}${McWilliams Center for Cosmology, Department of Physics, Carnegie Mellon University, Pittsburgh, PA 15213, USA}\\
$^{21}${School of Computer Science, Carnegie Mellon University, Pittsburgh, PA 15213, USA}\\
$^{22}${Kapteyn Astronomical Institute, University of Groningen, Postbus 800, 9700 AV, Groningen, The Netherlands}\\
...
Their magnification has been used as a natural telescope to observe otherwise undetectable objects \citep[e.g][]{2007ApJ...671.1196M,2017MNRAS.464.4823B,2016ApJ...833..264S}.   They have put limits on the self interaction of dark matter and on the theory of gravitation. \red{Citation?} Through microlensing they have been used to study the structure of quasars \citep{2008ApJ...689..755M,2008ApJ...673...34P,2011ApJ...729...34B}.   To access this wealth of information the first step is to find the lenses.
...
These are methods that classify the objects by making linear or nonlinear boundaries in a feature space.  The features are properties of the image and are typically chosen by the user with a combination of knowledge, intuition and trial-and-error.  Using the training set, an optimal boundaries are found according to a criterion that depends on the method.  The machine learns which combinations of features are best for distinguishing between lenses and non-lenses.

\red{Philippa, what do you think of this? }
...
\end{equation}
where $N$ is the number of training instances, $y \in \{0,1\}$ is the true class of the image and $\hat{y} \in [0,1]$ is the class probability predicted by the model. This optimization is usually performed by a Stochastic Gradient Descent (SGD) algorithm which updates the model iteratively by taking small gradient steps over randomly selected subsamples of the training set (so called \textit{mini-batches}). All the CNN-based methods presented in this work rely on the ADAM \citep{Kingma_2014} algorithm, an accelerated variant of SGD algorithm. \red{True?}

\fl{Things I still have to mention: activation function, dropout, batch normalisation}.


\red{Blue terms in following need to be defined.}
...
The network itself consists of three convolutional layers ($11\times 11 - 32$, $5\times 5 - 64$ and $3\times 3 - 64$), followed by two fully-connected layers ($256$ and $64$ neurons) and an output \blue{softmax layer}. The first five layers use the ELU (Exponential Linear Unit) \blue{activation function} \citep{2015arXiv151107289C}, which in our tests led to significantly faster convergence compared to ReLU and even SoftPlus activation. Dropout regularization \citep{2012arXiv1207.0580H,JMLR:v15:srivastava14a} is applied to both convolutional and fully connected layers, with ``keep'' probabilities $p=2/3$ and $p=1/2$, respectively.
...
To avoid the problem of the data flow distribution getting out of the comfort zone of the activation functions (\blue{Internal Covariate Shift}), we used a mix of normalization propagation \citep{norm_prop} (without the constraint on the weights but a proper initialization) and batch normalization \citep{batch_norm} (slowly disabled over the iterations).
...
Making batches of 30 images adds noise to the gradient \red{which?} makes the iterations faster.
...
\begin{itemize}
\item Random rotation of the image in the range [0, 180$\degree$], using a \blue{reflection fill mode} to keep constant the size of the images.
...
\red{This is a bit ragged and another submission needs to be analysed.}
...
\red{breakups Manchester between Neal and Amit}
